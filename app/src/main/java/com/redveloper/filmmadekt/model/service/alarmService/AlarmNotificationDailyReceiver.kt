...
class AlarmNotificationDailyReceiver : BroadcastReceiver(){
    override fun onReceive(context: Context?, intent: Intent?) {
        ...
        builder.setAutoCancel(true)
            ...
            .setContentText(context?.resources?.getString(R.string.notif_daily))
            .setDefaults(Notification.DEFAULT_LIGHTS.or(Notification.DEFAULT_SOUND))
            .setContentInfo(context?.resources?.getString(R.string.info))

        val notifyManager : NotificationManager = context?.getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager

        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.O){
            val channel = NotificationChannel(Constant.channelID, Constant.channelName, NotificationManager.IMPORTANCE_DEFAULT)
